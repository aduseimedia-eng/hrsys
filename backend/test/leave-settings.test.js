...
test('leave balance uses the configured entitlement and day-count rules', async () => {
  ...
  assert.deepEqual(balanceQuery.params, [44, 8, 2026, false, false]);
...
test('leave request listing exposes the configured annual balance calculation', async () => {
  ...
  assert.equal(res.statusCode, 200);
  assert.deepEqual(calls[1].params, [44, 30, false, false]);
  assert.match(calls[1].text, /\$2::int AS annual_entitlement/i);
  assert.match(calls[1].text, /generate_series/i);
  assert.match(calls[1].text, /employees e\s+ON e\.id = lr\.employee_id AND e\.company_id=lr\.company_id/i);
});
...
test('annual leave request enforces configured counted-day maximum before reserving balance', async () => {
  ...
  assert.deepEqual(dayCount.params, [44, '2099-01-05', '2099-01-12', false, false]);
  assert.equal(calls.at(-1).text, 'ROLLBACK');
});
...
test('annual leave request applies the configured entitlement to approved and pending days', async () => {
  ...
  assert.deepEqual(reservedCount.params, [44, 8, ['pending', 'approved'], 2099, true, true]);
  assert.equal(calls.at(-1).text, 'ROLLBACK');
});
...
test('annual leave approval rechecks the company policy before modifying the request', async () => {
  ...
  assert.deepEqual(approvedCount.params, [44, 8, ['approved'], 2026, false, true]);
  assert.equal(calls.at(-1).text, 'ROLLBACK');
  assert.equal(client.released, true);
});
