...
exports.getDashboard = async (req, res) => {
  try {
    ...
    const [
      ...
      db.query(
        "SELECT COUNT(*) FROM attendance WHERE company_id=$1 AND work_date = $2 AND status = 'present'",
        [req.user.company_id, today],
      ),
      db.query(
        "SELECT COUNT(*) FROM leave_requests WHERE company_id=$1 AND status='approved' AND $2 BETWEEN start_date AND end_date",
        [req.user.company_id, today],
      ),
      db.query(
        "SELECT SUM(net_salary) as total FROM payroll WHERE company_id=$1 AND month=$2 AND year=$3 AND status IN ('processed','paid')",
      ...
      db.query(
        `SELECT d.id, d.title, d.doc_type, d.expiry_date, CONCAT(e.first_name,' ',e.last_name) AS employee_name FROM documents d JOIN employees e ON e.id=d.employee_id WHERE d.company_id=$1 AND d.expiry_date BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '30 days' ORDER BY d.expiry_date ASC LIMIT 8`,
        [req.user.company_id],
      ),
    ]);
...
exports.getAll = async (req, res) => {
  try {
    ...
    if (role) {
      params.push(role);
      where += ` AND e.role = $${params.length}`;
    }
    if (employment_type) {
      params.push(employment_type);
      where += ` AND e.employment_type = $${params.length}`;
    }
    if (search) {
      params.push(`%${search}%`);
  ...
  try {
    ...
    if (
      ...
      return res
        ...
        .json({
          error:
            "Phone number is required so staff can complete first sign-in verification",
        });
    }
    ...
    await notifyEmployee({
      ...
      type: "welcome",
      message: `Welcome to the team, ${first_name}! Your account has been created.`,
    });
    ...
    if (
      err.code === "23505" &&
      err.constraint === "idx_employees_company_employee_code"
    )
  ...
  try {
    ...
    for (const field of allowedFields) {
      ...
      if (
        field === "role" &&
        value &&
        !["admin", "manager", "employee"].includes(value)
      ) {
      ...
      if (
        ...
        value &&
        !EMPLOYMENT_TYPES.includes(value)
      ) {
        return res.status(400).json({ error: "Invalid employment type" });
      }
      if (field === "employee_code") {
        ...
        if (!value || value.length > 50)
          return res
            ...
            .json({
              error:
                "Employee ID is required and must be 50 characters or fewer",
            });
      }
    ...
    const { rows } = await db.query(
      `UPDATE employees SET ${updates.join(", ")} WHERE id=$${params.length} AND company_id=$${params.length + 1}
       RETURNING id, employee_code, first_name, last_name, email, role, job_title, phone, address,
                 ...
                 emergency_contact_name, emergency_contact_relationship,
                 emergency_contact_phone, emergency_contact_address, bank_name, bank_account_name,
                 bank_account_number, bank_branch, linkedin_url, twitter_url, facebook_url, instagram_url,
...
exports.promote = async (req, res) => {
  ...
  if (!Object.keys(changes).length)
    return res
      .status(400)
      .json({ error: "Choose at least one employment detail to update" });
  if (changes.role && !["admin", "manager", "employee"].includes(changes.role))
    return res.status(400).json({ error: "Invalid role" });
  if (
    changes.employment_type &&
    !EMPLOYMENT_TYPES.includes(changes.employment_type)
  )
    return res.status(400).json({ error: "Invalid employment type" });
  if (
    changes.salary !== undefined &&
    (Number.isNaN(Number(changes.salary)) || Number(changes.salary) < 0)
  )
    return res.status(400).json({ error: "Salary must be a valid amount" });
  ...
  try {
    ...
    for (const field of allowed) {
      ...
      if (["department_id", "manager_id"].includes(field) && value === "")
        value = null;
    ...
    const { rows: updatedRows } = await client.query(
      `UPDATE employees SET ${assignments.join(", ")} WHERE id=$${params.length - 1} AND company_id=$${params.length} RETURNING *`,
      params,
    );
    const updated = updatedRows[0];
    const previousDetails = Object.fromEntries(
      allowed.map((field) => [field, current[field]]),
    );
    const newDetails = Object.fromEntries(
      allowed.map((field) => [field, updated[field]]),
    );
    await client.query(
      ...
      [
        ...
        employeeId,
        req.user.id,
        req.body.effective_date || new Date().toISOString().slice(0, 10),
        previousDetails,
        newDetails,
    ...
    await notifyEmployee({
      ...
      employeeId,
      type: "promotion",
      message: `Congratulations! You have been promoted to ${updated.job_title || "your new role"}.`,
    });
    res.json(updated);
  } catch (err) {
    await client.query("ROLLBACK");
...
exports.createDepartment = async (req, res) => {
  try {
    ...
    const existing = await db.query(
      "SELECT id FROM departments WHERE company_id=$1 AND LOWER(name)=LOWER($2)",
...
exports.deleteDepartment = async (req, res) => {
  try {
    ...
    const { rows } = await db.query(
      "DELETE FROM departments WHERE id=$1 AND company_id=$2 RETURNING id, name",
      [id, req.user.company_id],
    );
    if (!rows.length)
      return res.status(404).json({ error: "Department not found" });
